<?php
    ...
    if(isset($_POST['doc_login']))
    {
        ...
        if($rs) {
            ...
            // Nếu đăng nhập bác sĩ thất bại, kiểm tra đăng nhập cho dược sĩ
            $phar_number = $_POST['doc_number']; // Sử dụng chung trường input 'doc_number'
            $phar_pwd = sha1(md5($_POST['doc_pwd'])); // Sử dụng chung trường input 'doc_pwd'
            $stmt=$mysqli->prepare("SELECT phar_number, phar_pwd, phar_id FROM his_pharmacists WHERE phar_number=? AND phar_pwd=? ");//sql to log in user
            $stmt->bind_param('ss', $phar_number, $phar_pwd);//bind fetched parameters
            $stmt->execute();//execute bind
            $stmt -> bind_result($phar_number, $phar_pwd ,$phar_id);//bind result
            $rs=$stmt->fetch();
            if($rs) {
                $_SESSION['phar_id'] = $phar_id;
                $_SESSION['phar_number'] = $phar_number;//Assign session to phar_number id
                header("location:his_phar_dashboard.php");
            } else {
